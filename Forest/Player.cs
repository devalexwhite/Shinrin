...
namespace Forest
{
    public class Player
    {
        ...
        {
            //spriteBatch.Draw(Texture, Position, new Rectangle(0, 0, Width, Height), Color.White);

            if (movement.Y < 0)
            {
                walkingUp.Resume();
                walkingUp.Draw(spriteBatch, gameTime, Position);
            }
            else if (movement.Y > 0)
            {
                walkingDown.Resume();
                walkingDown.Draw(spriteBatch, gameTime, Position);
            }
            else if (movement.X < 0)
            {
                walkingLeft.Draw(spriteBatch, gameTime, Position);
            }
            else if (movement.X > 0)
            {
                walkingRight.Draw(spriteBatch, gameTime, Position);
            }
            else
            {
                walkingDown.Draw(spriteBatch, gameTime, Position);
        ...
        private void LoadContent(ContentManager contentManager)
        {
            ...
            walkingDown.Play(200);
            walkingUp.Play(200);
            walkingLeft.Play(200);
            walkingRight.Play(200);
